...
<div class="max-w-lg">
    ...
    <p class="text-slate-600 text-sm mb-6">Reassign owner if needed (cannot be a super admin account).</p>
    <div class="card-elevated p-8">
        <form method="POST" action="{{ route('admin.stores.update', $store) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
                ...
                <select name="owner_user_id" id="owner_user_id" required class="block w-full ring-sea">
                    @foreach($eligibleOwners as $o)
                        <option value="{{ $o->id }}" {{ (string) old('owner_user_id', $store->user_id) === (string) $o->id ? 'selected' : '' }}>{{ $o->name }} — {{ $o->email }}</option>
                    @endforeach
                </select>
                @error('owner_user_id')
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50/80 p-4 space-y-3">
                ...
                <p class="text-xs text-slate-500">Shop tab icon &amp; link previews for this store only.</p>
                @error('logo')
            ...
            <p class="text-xs text-slate-500">Customer shop: <a href="{{ route('shop.show', $store) }}" class="link-sea" target="_blank" rel="noopener">{{ $store->customerShopUrl() }}</a></p>
